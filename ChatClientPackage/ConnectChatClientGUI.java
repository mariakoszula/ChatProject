		...
		connectBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				if(!isIpAddressCorrect()){
					...
					ConnectToServer connectToServerTask = new ConnectToServer(serverIpAddress, portNumber);
			...
			private void showGivenValuesNotCorrectBox(String wrongValue) {
				String message = "Podana wartoœæ: "+wrongValue+" jest niepoprawna. Spróbuj jeszcze raz.";
				JOptionPane.showMessageDialog(null, message, "Niepoprawna wartoœæ", JOptionPane.WARNING_MESSAGE);
			}
			...
			private boolean isIpAddressCorrect() {
				if(serverIpAddress.matches(regexIP)) return true;
				else return false;
			}
